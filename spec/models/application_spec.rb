...
RSpec.describe Application, type: :model do
  ...
  describe "instance methods" do
    describe "confirm_adoptable" do
      xit "changes the status of all pets in application pets arguement to adoptable: false" do
        @application_pets = [@app_pet_1, @app_pet_2]
 
        expect(@app_pet_1.pet.adoptable).to eq(true)
        expect(@app_pet_2.pet.adoptable).to eq(true)

        @application_1.confirm_adoptable(@application_pets)

        expect(@app_pet_1.pet.adoptable).to eq(false)
        expect(app_pet_2.pet.adoptable).to eq(false)
      end
    end
    describe "update_status" do
      ...
      xit "calls confirm_adoptable which changes all pets status to adoptable:false if application is Approved!" do
        application_pets = [@app_pet_1, @app_pet_2]
        @app_pet_1.approved = true
        @app_pet_2.approved = true
        @application_1.update_status(application_pets)
